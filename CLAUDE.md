...
## Reglas de proceso
- La rama es por unidad de trabajo, no por petición. Antes de tocar código, mira en qué rama estás: si ya estás en una rama de trabajo —cualquiera que no sea `main` ni una `sN/*`—, sigue en ella. Solo si estás en `main` o en una `sN/*` creas una nueva (`git checkout -b feat/<slug>`). Nunca commitear directo en `main` ni en una `sN/*`.
- El commit sí es por petición: al cerrar cada una, usar la skill `/commit`.
- Al terminar la unidad de trabajo entera —no al cerrar cada petición—: `gh pr create` con una descripción completa de los cambios en el cuerpo del PR, y después el subagente `adversarial-reviewer` sobre él, antes de darlo por terminado. Las dos cosas van una sola vez.
- Al abrir el PR, no repitas ese resumen en el chat: la sesión se va a perder, el PR no. Responde solo con la URL del PR.